...
// Grouped tester interface
interface GroupedTester {
  testerId: string;
  email: string;
  sessionId: string;
  deviceInfo?: string;
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [testers, setTesters] = useState<GroupedTester[]>([]);
  ...
  // Approve ALL apps for a tester by email
  const handleApproveAll = async (email: string, status: string) => {
    if (!token) return;
    try {
      const data = await apiApproveAllTesterApps(token, email, status);
      
      if (data.success) {
        showCopyNotice(`Barua pepe ${email} imesasishwa kuwa ${status}`);
        loadDashboardData();
      }
    } catch (err) {
  ...
  const handleDeleteTester = async (testerId: string, email: string) => {
    if (!token) return;
    if (!window.confirm(`Je, una uhakika unataka kufuta tester huyu (${email})?`)) return;

    try {
      const data = await apiDeleteTester(token, testerId);
  ...
  const copyApprovedEmails = () => {
    ...
    testers.forEach(tester => {
      const hasApproved = tester.registrations.some(r => r.status === 'approved');
      if (hasApproved) {
        approvedEmails.add(tester.email);
      }
    });
    ...
    const emailList = Array.from(approvedEmails).join(', ');
    navigator.clipboard.writeText(emailList);
    showCopyNotice(`Barua pepe ${approvedEmails.size} zimenakiliwa tayari kwa ajili ya Google Play Console!`);
  };
  ...
  const filteredTesters = testers.filter(tester => {
    const matchesSearch = 
      ...
      tester.sessionId.toLowerCase().includes(searchQuery.toLowerCase()) ||
      tester.testerId.toLowerCase().includes(searchQuery.toLowerCase());
    
    let matchesStatus = true;
    if (statusFilter !== 'all') {
      matchesStatus = tester.registrations.some(r => r.status === statusFilter);
    }
  ...
  const getPrimaryStatus = (tester: GroupedTester): string => {
    if (tester.registrations.some(r => r.status === 'approved')) return 'approved';
    if (tester.registrations.some(r => r.status === 'pending')) return 'pending';
    if (tester.registrations.some(r => r.status === 'rejected')) return 'rejected';
    return 'pending';
  };

  const approvedCount = testers.filter(t => getPrimaryStatus(t) === 'approved').length;
  const pendingCount = testers.filter(t => getPrimaryStatus(t) === 'pending').length;
  const rejectedCount = testers.filter(t => getPrimaryStatus(t) === 'rejected').length;
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3.5 rounded-2xl bg-slate-850 border border-slate-800">
                    <div className="relative w-full sm:w-72">
                      ...
                      <input
                        type="text"
                        placeholder="Tafuta Gmail au Session ID..."
                        value={searchQuery}
                    ...
                    <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                      ...
                      >
                        ...
                        <span>Nakili Approved ({approvedCount})</span>
                      </button>
                    </div>
                  </div>

                  {/* Testers Table - Grouped by Email */}
                      ...
                      <table className="w-full text-left text-xs">
                        <thead className="bg-slate-800 text-slate-300 uppercase font-bold tracking-wider text-[10px]">
                          <tr>
                            ...
                            <th className="px-4 py-3.5">Apps Registered</th>
                        ...
                        <tbody className="divide-y divide-slate-800">
                          {filteredTesters.length === 0 ? (
                            ...
                            filteredTesters.map(tester => {
                              const primaryStatus = getPrimaryStatus(tester);
                              return (
                                <tr key={tester.testerId} className="hover:bg-slate-800/50 transition-colors">
                                  <td className="px-4 py-3 font-medium text-white">
                                    ...
                                    {tester.deviceInfo && (
                                      <div className="text-[10px] text-slate-400 mt-0.5">
                                        Simu: {tester.deviceInfo}
                                      </div>
                                    )}
                                  </td>
                                  <td className="px-4 py-3 text-slate-300">
                                    <span className="font-semibold">{tester.registrations.length}</span>
                                    <span className="text-slate-500"> / 6 apps</span>
                                  </td>
                                  <td className="px-4 py-3 text-slate-400 whitespace-nowrap">
                                    {new Date(tester.createdAt).toLocaleString('sw-TZ', { 
                                      day: 'numeric', 
                                      month: 'short', 
                                      year: 'numeric',
                                      hour: '2-digit',
                                  ...
                                  <td className="px-4 py-3">
                                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold ${
                                      ...
                                      {primaryStatus === 'approved' && <CheckCircle2 className="w-3 h-3" />}
                                      {primaryStatus === 'pending' && <Clock className="w-3 h-3" />}
                                      {primaryStatus === 'rejected' && <XCircle className="w-3 h-3" />}
                                      <span className="capitalize">{primaryStatus}</span>
                                    </span>
                                  </td>
                                  <td className="px-4 py-3 text-right">
                                    <div className="inline-flex items-center gap-1.5">
                                      {primaryStatus !== 'approved' && (
                                        <button
                                          onClick={() => handleApproveAll(tester.email, 'approved')}
                                          className="px-2.5 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/20 transition-colors cursor-pointer font-bold"
                                          title="Idhinisha Apps Zote"
                                        >
                                          ✓ Idhinisha Zote
                                        </button>
                                      )}
                                      ...
                                      {primaryStatus !== 'rejected' && (
                                        <button
                                          onClick={() => handleApproveAll(tester.email, 'rejected')}
                                          className="px-2.5 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/25 text-red-400 border border-red-500/20 transition-colors cursor-pointer font-bold"
                                          title="Kataa Apps Zote"
                                        >
                                          ✗ Kataa
                                        </button>
                                      )}
                                      ...
                                      <button
                                        onClick={() => handleDeleteTester(tester.testerId, tester.email)}
                ...
                <div className="space-y-6">
                  <div className="p-4 rounded-2xl bg-slate-850 border border-slate-800">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Kila app ina Google Play Closed Testing URL yake. Tester anapoidhinishwa (Approved), tovuti itaonyesha kiungo hiki kwake.
                    </p>
                  </div>
